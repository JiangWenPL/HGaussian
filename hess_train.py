...
@torch.no_grad()
def save_checkpoint(gaussians, iteration, scene, save_path=None, save_last=True):
    ckpt_dict = {"model_params": gaussians.capture(), "iteration": iteration, }
...
def load_checkpoint(ckpt_path: str, gaussians, scene, opt):
    ckpt_dict = torch.load(ckpt_path)
    (model_params, iteration) = ckpt_dict["model_params"], ckpt_dict["iteration"]
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if checkpoint:
        if os.path.exists(checkpoint):
            first_iter = load_checkpoint(checkpoint, gaussians, scene, opt)
        else:
...
if __name__ == "__main__":
    ...
    wandb.init(project='hess', resume="allow", entity='grasp-3dv', id=os.path.split(args.model_path.rstrip('/'))[-1], config=vars(args))
